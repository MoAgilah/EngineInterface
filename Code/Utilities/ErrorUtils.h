...
namespace err {
    ...
    inline std::string format_with_location(std::string_view msg,
        const char* file,
        int line)
    {
        return std::format("{} ({}:{})", msg, file, line);
    }
    ...
    inline void report_error(std::string_view msg,
        ...
        int line)
    {
        set_last_error(format_with_location(msg, file, line));
        log_error(std::string(msg), func, file, line);
    }
        ...
        bool setLastError = true)
    {
        ...
        log_warn(std::string(msg), func, file, line);
    }
...
#define THROW_IF_FALSE(expr)                                                                \
    do {                                                                                    \
        if (!(expr)) {                                                                      \
            ...
            ::err::report_error(__msg, __func__, __FILE__, __LINE__);                       \
